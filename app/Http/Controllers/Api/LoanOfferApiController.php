    ...
    public function latestPaymentTransactions(Request $request): JsonResponse
{
    ...
    $query = LoanPayment::with(['client', 'loan', 'agent'])
    ...
    public function latestPaymentTransactions222(Request $request): JsonResponse
{
    ...
    // Corrected 'paid_at' to 'payment_date'
    $transactions = LoanPayment::with(['client', 'loan', 'agent'])
        ->orderBy('payment_date', 'desc')
        ->paginate($perPage);
     ...
     */
   public function clientLoanspayHistory2(Request $request, $id): JsonResponse
{
    try {
        // Retrieve the loan IDs associated with the client
        $loanIds = UserLoan::where('client_id', $id)->pluck('id');

        // Retrieve loan payments for those loan IDs
        $clientLoanPayments = LoanPayment::whereIn('loan_id', $loanIds)->get();

        return response()->json([
            'status' => 'success',
            'data' => $clientLoanPayments
        ], 200);
    } catch (\Exception $e) {
        return response()->json([
            'status' => 'error',
...
// client loan payment history api
     ...
     */
    public function createLoan101(Request $request): JsonResponse
    {
        // Validate the request data
        $validator = Validator::make($request->all(), [
            'user_id' => 'required|exists:users,id',
     ...
     * GET /dusers/{id}/loans
     */
    public function userLoansList10(Request $request, $id): JsonResponse
    {
        try {
     ...
     */
    public function destroy2Z($id): JsonResponse
    {
        try {
            $loanOffer = LoanOffer::findOrFail($id);
            $loanOffer->delete();
    ...
    public function all_loans(Request $request): JsonResponse
    {
        try {
            ...
            // Define the base query with eager loading of agent and client
            $query = UserLoan::with(['agent', 'client']); // Assuming 'agent' and 'client' are the relationship names
    
            // Check if there is a search parameter and modify the query accordingly
            if ($request->search) {
                $query->where('trx', 'like', '%' . $request->search . '%');
            ...
            // Structure the response with agent and client details
            $loanData = $loans->map(function ($loan) {
                return [
                    ...
                    'status' => $loan->status,
                    'agent' => [
                        'id' => $loan->agent->id ?? null,
                        'name' => ($loan->agent->f_name ?? 'Unknown') . ' ' . ($loan->agent->l_name ?? ''),
    
                        'phone' => $loan->agent->phone ?? 'N/A',
                    ],
                    'client' => [
                        'id' => $loan->client->id ?? null,
                        'name' => $loan->client->name ?? 'Unknown',
    ...
    public function paidLoans(Request $request): JsonResponse
    {
        try {
            ...
            // Define the base query with eager loading of agent and client
            $query = UserLoan::paid()->with(['agent', 'client']); // Assuming 'agent' and 'client' are the relationship names
    ...
    public function pendingLoans(Request $request): JsonResponse
    {
        try {
            ...
            // Define the base query with eager loading of agent and client
            $query = UserLoan::pending()->with(['agent', 'client']); // Assuming 'agent' and 'client' are the relationship names
    ...
    public function rejectedLoans(Request $request): JsonResponse
    {
        try {
            ...
            // Define the base query with eager loading of agent and client
            $query = UserLoan::rejected()->with(['agent', 'client']);
    ...
    public function runningLoans(Request $request): JsonResponse
    {
        try {
            ...
            // Define the base query with eager loading of agent and client
            $query = UserLoan::running()->with(['agent', 'client']);
     ...
     */
    public function loanplansindex1010(): JsonResponse
    {
     ...
     * GET /dloans/{id}/show
     */
    public function showLoan10($id): JsonResponse
    {
        try {
            // Fetch the loan by ID
            $loan = UserLoan::findOrFail($id);
     ...
     */
    public function update10(Request $request, $id): JsonResponse
    {
        // Validate the request inputs
        $validator = Validator::make($request->all(), [
            'name' => 'sometimes|required|string|max:255',
