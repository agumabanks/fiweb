    ...
    public function SmsNotification(Request $request)
        {
            try {
                ...
                $smsText = "Dear {$transaction->client->name}, Loan payment notification: {$transaction->amount} paid. Your Balance is {$transaction->credit_balance}. Thank you for choosing Maslink  Credit Services! Contact us: 0759670263(Benon) 0707670776(Moses)";
        
                // Construct the full API URL with the query parameters
                $fullUrl = $url . '?api_key=' . $apiKey . '&sender=' . $sender . '&contacts=' . '+256706272481' . '&text=' . urlencode($smsText);
